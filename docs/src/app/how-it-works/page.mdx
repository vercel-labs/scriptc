...
- **Concurrency** — `async` functions compile to stackless coroutines, so a suspended call keeps only its live state in a small heap frame, with JS-exact scheduling: microtasks drain in the same order Node's do, timers fire in the same order, and the event loop (kqueue on macOS, epoll on Linux) has no external dependencies. On native targets, generators and programs that fall back to the C backend still run on stackful fibers.
